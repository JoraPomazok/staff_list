		...
		block: function()
		{
			return {
				format: function(tag, attr, value, type)
				{
                    ...
                    var id  = selection.anchorNode.id ? selection.anchorNode.id : '=';
                    var pid = selection.anchorNode.parentNode.id ? selection.anchorNode.parentNode.id : '=';
                    // console.log('format: ',selection.getRangeAt(0));
                    // console.log('format: ',selection.anchorNode.id,selection.anchorNode.parentNode.id);
                    if(!id.match(/redactor-uuid/) && !pid.match(/redactor-uuid/))
					{
                        return false;
					}
                    ...
                    // console.log(this.utils.isCollapsed());
				...
				formatUncollapsed: function(tag, attr, value, type)
				{
                    ...
                    var selection= this.selection.get();
                    console.log('formatUncollapsed: ',selection.getRangeAt(0));
                    if(selection.anchorNode.parentNode.id
					&& selection.anchorNode.parentNode.id.match(/redactor-uuid/))
                    {
...
// return;
                    ...
                    // if (blocks[0] && ($(blocks[0]).hasClass('redactor-in') || $(blocks[0]).hasClass('redactor-box') || $(blocks[0]).parents('.redactor-in').length	|| $(blocks[0]).parents('redactor-box').length	))
                    // {
		...
		build: function()
		{
			return {
				...
				{
					this.$editor = $('<div contenteditable="true" class="js_omni_redactor_container"/>');
					...
					// place
					this.$box.insertAfter(this.$element).append(this.$element);
					this.$box.append(this.$editor);
					if(!this.opts.pasteOmniPlainText)
                    {
			...
			return {
				...
				build: function(btnName, btnObject)
				{
                    ...
                    if (typeof btnObject.icon !== 'undefined' && !this.opts.buttonsTextLabeled)
                    {
    					...
    					if(btnName=='pre')
                        {
                            $icon.addClass('fa').addClass('fa-code');
                        }
                        else
						{
		...
		clean: function()
		{
			return {
				...
				{
					// if paste event
                    html = html.replace(/<o:p>[\s\S]*?<\/o:p>/gi, '<br>');
                    if (insert !== true)
					{
                    ...
                    //Вставим класс для таблиц
                    html = html.replace(/<table/gi, '<table class="omni_redactor_table" style="border-collapse: collapse;font-size: 1em;width: 100%;" ');
                    html = html.replace(/<td/gi, '<td style="padding: 5px; border: 1px solid #ddd;vertical-align: top;" ');
					return html;
		...
		code: function()
		{
			return {
				...
				startSync: function(html)
				{
					...
					// clean
					html = this.utils.preEncodePre(html);
					html = this.clean.onSync(html);
                    html = this.utils.postEncodePre(html);
		...
		insert: function()
		{
			return {
				...
				placeNode: function(node, deleteContent)
				{
					...
					if(!$(sel.anchorNode).hasClass('redactor-layer'))
                    {
                    	var html = this.core.editor().html();
                    	if(!html.length)
                        {
...
//					on Shift+Enter or Ctrl+Enter
					if (key === this.keyCode.ENTER && !e.shiftKey && !e.ctrlKey && !e.metaKey)// && (e.ctrlKey || e.shiftKey))
					{
                        if(this.keydown.block.tagName != 'LI'
						&& this.keydown.block.tagName != 'PRE')
						{
							e.preventDefault();
                        }
				...
				onShiftEnter: function(e)
				{
                    ...
                    else if (this.keydown.block.tagName != 'LI')
					{
				...
				wrapToParagraph: function()
				{
                    ...
                    //omni
                    $current.replaceWith($current[0]);
                    this.caret.end($current);
                    return;
		...
		link: function()
		{
			return {
				...
				buildLinkFromModal: function()
				{
                    ...
                    if(link.url.match(/^\[[A-z_]+\]$/) && link.url.match(/url/))
					{
						return link;
					}
					// parse
				...
				templates: function()
				{
					this.opts.modal = {
						...
						+ '<div class="redactor-modal-tab" data-title="General">'
							+ '<section>'
								+ '<label>URL</label>'
								+ '<input type="text" id="redactor-link-url" aria-label="URL" />'
							+ '</section>'
							+ '<section>'
		...
		paragraphize: function()
		{
			return {
				...
				getSafes: function(html)
				{
					...
					// remove paragraphs in blockquotes (omni)
			...
			return {
				...
				insert: function(html, data)
				{
					...
					else
					{
                        ...
                        if(!html.match(/<br/))
						{
//                            html = this.clean.replaceNlToBr(html)
						}
		...
		utils: function()
		{
            tmp_pre_arr : {};
            return {
				preEncodePre : function (html) {
					var self = this;
                    this.utils.tmp_pre_arr = {};
                    return html.replace(/<pre([^>]*)>([\s\S]*)<\/pre>/gim,function (m0,attr,content) {
                        var strrand;
                        do {
                            strrand =Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
                        } while (self.utils.tmp_pre_arr[strrand]);
                        strrand = strrand.toString();
                        self.utils.tmp_pre_arr[strrand] = content;
                        return '<pre'+attr+ '>'+strrand+ '</pre>';
                    });
                },
				postEncodePre :function (html) {
                    var self = this;
                    // /"aaa\nvb\nhhhhh\nvb".replace(/vb(.*)vb/gim,'fuck');
                    // console.log(html );
                    return html.replace(/<pre([^>]*)>([\s\S]*)<\/pre>/gim,function (m0,attr,content) {
                        // console.log(content);
                        content = content.replace("\n",'');
                        return '<pre'+attr+ '>'+self.utils.tmp_pre_arr[content]+ '</pre>';
                    });
