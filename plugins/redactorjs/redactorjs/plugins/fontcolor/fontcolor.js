...
RedactorPlugins.fontcolor = {
	init: function()
	{
		...
		for (var i = 0; i < 2; i++)
		{
			...
			var $dropdown = $('<div class="redactor_dropdown redactor_dropdown_box_' + name + '" style="display: none; width: 264px;">');

			this.pickerBuild($dropdown, name, colors);
			$(this.$toolbar).append($dropdown);

			var btn = this.buttonAddAfter('fontsize', name, this.opts.curLang[name], $.proxy(function(btnName, $button, btnObject, e)
			{
	...
	pickerBuild: function($dropdown, name, colors)
	{
		...
		var $elNone = $('<a href="#" style="display: block; clear: both; padding: 6px 6px 6px; font-size: 12px; line-height: 1;"></a>')
