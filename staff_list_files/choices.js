...
var DEFAULT_CONFIG = {
  ...
  loadingText: 'Loading...',
  noResultsText: 'Нет результатов', // Правка. Перевёл на рус., чтобы не задавать параметр каждый раз при ините
  noChoicesText: 'Нет вариантов',   // Правка. Перевёл на рус., чтобы не задавать параметр каждый раз при ините
...
function () {
  ...
  _createClass(Choices, [{
    ...
    value: function hideDropdown(preventInputBlur) {
      ...
      const el = this.choiceList.element.closest(".choices").querySelector(".nano-pane") !== null ? this.choiceList.element.closest(".choices").querySelector(".nano-pane").offsetHeight : ''
      // const el = this.choiceList?.element.closest(".choices").querySelector(".nano-pane")?.offsetHeight
      ...
      // const scrollBar = target.closest(".choices").querySelector(".nano-pane.active")
      // console.log(target.closest(".choices").querySelector(".nano-pane.active"))
    ...
    value: function _canAddChoice(activeItems, value) {
      ...
      /** Правка. Если ничего не введено - не отображаем уведомление "Press Enter to add" START */
      if (value == 0) {
        canAddItem = ''
      } else {
        canAddItem = this._canAddItem(activeItems, value);
      }
      /** Правка. Если ничего не введено - не отображаем уведомление "Press Enter to add" END */
    ...
    value: function _searchChoices(value) {
      ...
      /**
       * Правка START. Видимо в этом форке сломан/недоработан поиск при добавлении своей опции.
       * В результате может быть так, что если мы ввели метку "Прочееее", а у нас есть метка "Прочее",
       * то будет показываться в выпадающем списке метка "Прочее", хотя уже нужно предложить юзеру добавить
       * такую метку выведя специальный div с текстом.
       * Чтобы вернуть как было: раскомментируйте код выше и закомментируйте код ниже.
      */

      var results = []
      haystack.filter(element => {
        if (element.label.toLowerCase().includes(newValue.toLowerCase())) {
          const item = []
          item.item = element
    ...
    value: function _onEnterKey(_ref4) {
      ...
      /**
       * Правка START. Если мы ввели метку и нажали Enter, а она есть в списке опций,
       * то она добавится, а уже существующая останется.
       * Таким образом можно добавить одну и ту же метку два раза.
       * Для того, чтобы вернуть как всё было уберите всё, что связано с option.
       * P.S срабатывает и в одиночном селекте.
      */

      const option = this._presetChoices.some(element =>
        // element.label.toLowerCase().includes(target?.value?.toLowerCase())
        element.label.toLowerCase().includes(target.value.toLowerCase())
      )
      ...
      if (hasActiveDropdown) {
        ...
        if (highlightedChoice) {
          if (addedItem) {
            this.unhighlightAll();
          //  Если добавляем свою метку
          } else {
            if (addedItem == false) {return }
            // add enter keyCode value
            if (activeItems[0]) {
              activeItems[0].keyCode = enterKey; // eslint-disable-line no-param-reassign
            }
            this._handleChoiceAction(activeItems, highlightedChoice);
            // Если выбираем из списка меток
          }
        }
    ...
    value: function _onClick(_ref10) {
      ...
      if (clickWasWithinContainer) {
        if (!this.dropdown.isActive && !this.containerOuter.isDisabled) {
          ...
          !target.matches(".nano") && !target.tagName === "SELECT") {
          /**
           * Правка. Добавил: !target.matches(".nano") && !target.tagName === "SELECT" , а то скрывается выпадающий список при клике на кастомный скролл
          */
          this.hideDropdown();
        }
      } else {
        /** Правка. Закомментил эту строку т.к "обновляются" айтемы в выпадающем списке. Например если навешал класс - он сбросится
          *
...
function () {
  ...
  _createClass(Dropdown, [{
    ...
    value: function hide() {
      ...
      // const el = this.element.closest(".choices").querySelector(".nano-pane.active")
  ...
  _createClass(Input, [{
    ...
    value: function setWidth(enforceWidth) {
      ...
      var callback = function callback(width) {
        ...
        /**
         * Правка. Изначально здесь была просто строка _this.element.style.width = width;
         * if нужен для того, чтобы при добавлении собственной метки сбрасывать ширину input,
         * в который добавляется метка, а иначе появляется баг, из-за которого
         * input остаётся с указанным width. Как результат - на следующую строку может перенестить просто пустая строка
        */
        if (_this.element.value.length === 0) {
          _this.element.style.width = "0px";
        } else {
          /**
           * Правка. Длинная опция введённая юзером рвёт шаблон
           */
          if (parseInt(width) >= _this.element.parentElement.clientWidth) return
