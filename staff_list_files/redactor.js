...
if (typeof Translate !== 'function') { var Translate = function(str){return str;}
}
(function() {
var Ajax = {};
...
$R.opts = {
    ...
    regex: {
        ...
        // url: /https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&//=]*)/gi omnifix ниже для таких ссылок https://service.ferroli.ru/products/УТ-00001181
        // url: /https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&//=].\S*)/gi omnifix ниже для таких ссылок https://service.ferroli.ru/products/УТ-00001181 https://котельныйсервис.рф/УТ-00001181 и тд
        url: /https?:\/\/(www\.)?[-\wа-яА-ЯЁё0-9@:%._+~#=]{1,256}\.[\wа-яА-ЯЁё0-9()]{1,6}([-\wа-яА-ЯЁё0-9()@:%_+.~#?&//=]*)/gi,
        aurl1: /(\b(https?|ftp):\/\/[-A-ZА-ЯЁё0-9+&@#\/%?=~_|!:,.;]*[-A-ZА-ЯЁё0-9+&@#\/%=~_|])/gim,
        aurl2: /(^|[^\/])(www\.[\S]+(\b|$))/gim
    },
    input: true,
    zindex: false,
...
$R.buttons = {
    html: {
        title: Translate('redactor_js/html_title'),
        icon: true,
...
$R.add('service', 'selection', {
    ...
    getElement: function(el)
    {
        ...
        while (node)
        {
            ...
            if(node.className && node.className.match(/redactor-box/))
            {
                break;
            }
        }
...
$R.add('service', 'cleaner', {
    ...
    input: function(html, paragraphize, started)
    {
        ...
        // converting entity
        html = html.replace(/\$/g, '&#36;');
//        html = html.replace(/&amp;reg/g, '[omni:replacerjs1]');
        html = html.replace(/&amp;/g, '&');
//        html = html.replace(/\[omni:replacerjs1\]/g, '&amp;reg');

        // convert to figure
        var converter = $R.create('cleaner.figure', this.app);
        // Omni inputConvertBefore event for table convert
        html = this.app.broadcast('inputConvertBefore', html);
    ...
    paste: function(html)
    {
        ...
        if (html.match(/urn:schemas-microsoft-com:office:excel/))
        {
            // OMNI FIX
...
$R.add('class', 'cleaner.figure', {
    ...
    convert: function(html, rules)
    {
        ...
        // convert
        $wrapper.find('img:not(.user-thmb)').each(this._convertImage.bind(this));
...
$R.add('service', 'insertion', {
    ...
    insertHtml: function(html, clean)
    {
        ...
        // Omni comment - fix, adding inline elements, need to use (isInsertedText)
        // breakline div
...
$R.add('service', 'utils', {
    ...
    {
        html = typeof html === 'string' ? html.replace('<br><br><!--StartFragment-->','').replace('<!--EndFragment--><br><br>','') : html;
        var div = this.createTmpContainer(html);
        //Омни: вставка из гугл док, баг "новаяя  сттрока"
        if (this.opts.breakline)
        {

            // console.log(div);
            $R.dom(div).find('p').each(function(node)
            {
                var $node = $R.dom(node);
                // console.log($node);
...
$R.add('module', 'contextbar', {
    ...
    _buildPosition: function(e, $el, position)
    {
        ...
        // omni fix redactor-context-toolbar-0 position
        // console.log($($el.nodes[0]).data('redactor-type'))

        top = (e.clientY + docScrollTop - barHeight) - height;
        ...
        let tempLeft = 0;
        if(left - leftFix > 0) {
            tempLeft = left - leftFix
        } else if(left + barWidth/2 + 150 > this.editor.getElement().outerWidth()) {
            tempLeft = left - (left - this.editor.getElement().outerWidth()) - 100
        }


        if($el.nodes.length) {
            let focus_el_type = $($el.nodes[0]).data('redactor-type');
            if(focus_el_type) {
                if(focus_el_type == 'image' || focus_el_type == 'video') { //код в ифе базовый от редактора, что выше и ниже - кастом
                    if (!position)
                        {
        ...
        return { top: (top - topFix) + 'px', left: tempLeft + 'px' };
    }
});
...
$R.add('class', 'toolbar.fixed', {
    ...
    _doFixed: function()
    {
        ...
        {
            !$container.parent().hasClass('has-fixed-toolbar') ? $container.parent().addClass('has-fixed-toolbar') : null
            !$('.wdt-emoji-popup').hasClass('custom-fixed') ? $('.wdt-emoji-popup').addClass('custom-fixed topEmoji') : null
        ...
        {
            $container.parent().removeClass('has-fixed-toolbar');
            $('.wdt-emoji-popup').removeClass('custom-fixed topEmoji');
