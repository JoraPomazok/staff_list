...
{this.g_url='https://omnidesk.ru';this.g_iframe_button_id='omni_widget_iframe',this.g_iframe_widget_id='omni_widget_iframe_widget',this.g_not_iframe_hint_id='omni_widget_hint',this.g_not_iframe_tip_id='omni_widget_tip',this.g_config={},this.config_widget=null,this.email_widget=null,this.widget={},this.g_iframe_button=null,this.g_not_iframe_hint=null,this.g_not_iframe_tip=null,this.g_iframe_widget=null,this.type_widget=null,this.b_send=!1;this.help_size={};this.btn_inited=!1;this.btn_prev_w=0;this.btn_prev_h=0;this.b_rendered_button=!1;this.yaCounterId=!1;this.support_lang=!1;for(var j in obj){this[j]=obj[j]}
...
{var body=document.getElementsByTagName("body")[0];self.g_iframe_button=document.createElement('iframe');self.g_iframe_button.id=self.g_iframe_button_id;self.g_iframe_button.name=self.g_iframe_button_id+'_name';self.g_iframe_button.scrolling="no";self.g_iframe_button.style.position='absolute';self.g_iframe_button.style.top='-1000px';self.g_iframe_button.style.left='-1234px';self.g_iframe_button.style.width='500px';self.g_iframe_button.style.height='500px';self.g_iframe_button.target='_top';self.g_iframe_button.src=self.g_url+'/client_widgets/init/'+self.g_config.widget_id+'?btn_hide='+(self.g_config.diplay_button?0:1)+'&lang='+self.GetBrowserLang();body.appendChild(self.g_iframe_button,body);self.g_not_iframe_hint=document.createElement('div');self.g_not_iframe_hint.id=self.g_not_iframe_hint_id;self.g_not_iframe_hint.name=self.g_not_iframe_hint_id+'_name';self.g_not_iframe_hint.style.position='absolute';self.g_not_iframe_hint.style.top='-1000px';self.g_not_iframe_hint.style.left='-1234px';self.g_not_iframe_hint.style.width='500px';self.g_not_iframe_hint.style.height='500px';self.g_not_iframe_hint.innerHTML='Я новая подсказка. Я жива!!!';body.appendChild(self.g_not_iframe_hint,body);self.g_iframe_widget=document.createElement('iframe');self.g_iframe_widget.id=self.g_iframe_widget_id;self.g_iframe_widget.name=self.g_iframe_widget_id+'_name';self.g_iframe_widget.style.position='absolute';self.g_iframe_widget.style.top='-4000px';self.g_iframe_widget.style.left='-4000px';if(!self.g_config.diplay_button){var body=document.getElementsByTagName("body")[0];self.g_iframe_widget.src=self.g_url+'/client_widgets/widget/'+self.g_config.widget_id+'?lang='+self.GetBrowserLang()+'&b_inited_user='+(window._g_omni_b_inited_user?1:0);body.appendChild(self.g_iframe_widget,body)}},this.RenderButton=function(btn_type,params,show_hint)
...
{var css_arr={'background':'transparent','border':'none','position':'fixed','z-index':'1000','width':'100%','height':'100%','top':'0px','bottom':'0px','left':'0px','right':'0px','margin_top':'0px'};var body=document.getElementsByTagName("body")[0];body.style.cssText="overflow-y: hidden";var new_this=this;setTimeout(function(){new_this.setCss(self.g_iframe_widget,css_arr);new_this.Show(self.g_iframe_widget)},700);if(window.outside_omni_chatra){window.frames[self.g_iframe_widget_id+'_name'].postMessage('offOmniChatra',self.g_url)}
window.frames[self.g_iframe_widget_id+'_name'].postMessage('update_content_field',self.g_url);setTimeout(function()
{self.g_iframe_widget.contentWindow.focus();self.g_iframe_widget.focus()},1000)},this.RenderHint=function(params,position)
...
{self.g_iframe_widget.src=self.g_url+'/client_widgets/widget/'+self.g_config.widget_id+'?lang='+self.GetBrowserLang()+'&b_inited_user='+(window._g_omni_b_inited_user?1:0);self.b_send=!1}};this.listen_iframe=function(e){var self_=self;if(e.data&&e.origin.match(/vk\.com/))
...
this.Show(document.querySelectorAll('div#chatra')[0]);Chatra('show');this.Hide(self.g_iframe_button)}
...
var s=document.createElement('script');s.async=!0;s.src=(document.location.protocol==='https:'?'https:':'http:')+'//vk.com/js/api/openapi.js?141';if(document.head)document.head.appendChild(s);var vk_interval=setInterval(function(d){if(!window.VK)
...
{(w[c].q=w[c].q||[]).push(arguments)};s.async=!0;s.src=(d.location.protocol==='https:'?'https:':'http:')+'//call.chatra.io/chatra.js';if(d.head)d.head.appendChild(s)})(document,window,'Chatra');if(!window.outside_omni_chatra){Chatra('updateIntegrationData',window.ChatraIntegration);setTimeout(function(){self_.Hide(document.querySelectorAll('div#chatra')[0]);Chatra('openChat',window)},300)
setTimeout(function(){Chatra('minimizeWidget');self_.Hide(document.querySelectorAll('div#chatra')[0]);if(Chatra._chatHiddenByFrame)
...
window.frames[self_.g_iframe_widget_id+'_name'].postMessage('render_widget_'+data.type,self_.g_url);self_.RenderWidget();if(self_.config_widget.type=="knowledge_mail"&&self_.config_widget.settings.knowledge_type=="search")
...
{var body=document.getElementsByTagName("body")[0];self_.g_iframe_widget.src=self_.g_url+'/client_widgets/widget/'+self_.g_config.widget_id+'?lang='+self.GetBrowserLang()+'&b_inited_user='+(window._g_omni_b_inited_user?1:0);body.appendChild(self_.g_iframe_widget,body)}
window.outside_omni_chatra=Array.from(document.head.getElementsByTagName('script')).some(function(item){return item.src.match('call.chatra.io/chatra.js')!==null});if(window.outside_omni_chatra){window.frames[self_.g_iframe_button_id+'_name'].postMessage('offOmniChatra',self_.g_url)}
...
return!1}
this.GetBrowserLang=function(){var language=window.navigator?(window.navigator.language||window.navigator.systemLanguage||window.navigator.userLanguage):"ru";if(self.support_lang)
{language=self.support_lang}
return language.substr(0,2).toLowerCase()}};var OmniWidgetApi={call:function(str,data){str=str.split('.');var tmp=Object.keys(cOmni);var cW=str[1]&&cOmni[str[1]]?cOmni[str[1]]:cOmni[tmp[0]]
...
{if(data)
{let new_data=JSON.stringify(data).replace(/["']/g,"'");window.frames[cW.g_iframe_widget_id+'_name'].postMessage('set_fields_after'+new_data,cW.g_url)}}
else if(key=='reset')
